...
if __name__ == "__main__":
    ...
    # Pos_est[0] = 0
    # for i in range(0, len(Pos_est)-1):
    #     motor_displacement[i+1] = abs(Pos_est[i+1]-Pos_est[i]) + motor_displacement[i]
    ...
    # print(mouse_displacement)
    # mouse_displacement2 = [x/CPI*2.54/r for x in mouse_displacement]
    # # print(mouse_displacement2)
    # plt.scatter(MouseTime[0:len(MouseTime)-1], mouse_displacement2[0:len(mouse_displacement2)-1], color='r', label="Mouse Position", s=4)
    # # print("latency = ", mouse_displacement[0:len(mouse_displacement)-1] - Pos_est[0:len(Pos_est)-1])
    # plt.legend(loc="lower center")
    ...
    # output_data2 = np.concatenate((np.array(mouseX/CPI).reshape(-1, 1), np.array(mouseY/CPI).reshape(-1, 1), np.array(MouseTime).reshape(-1, 1), np.array(mouse_displacement2).reshape(-1, 1)), axis=1)
    # name1 = './build/Estimate_Data/Est_Data_' + path1[0].replace('build/Raw_Data/', '')
    # name2 = './build/Estimate_Data/Est_Data_' + path2[0].replace('build/Raw_Data/', '')
    # np.savetxt(name1, output_data1, delimiter=',')
    # np.savetxt(name2, output_data2, delimiter=',')
    # print('Txt file save successfully !')
    
    print("kf variance :", np.var(Acc_KF_est))
