...
class KalmanFilter:
    ...
    # def KF(self, measurement):
        ...
        self.x_true_data.append(self.x_k1.flatten())
        # print("self.x_k1 =", self.x_k1)
    ...
    def getKFData(self):
        ...
        x_true_data_noise = x_true_data_noise.reshape(1500, 1)
...
#---------------------------------------輸出數據.txt-------------------------------------------#
    x_data_all, P_data_all = kf.getKFData()
    np.savetxt('./sim_data/x_data_all.txt', x_data_all, delimiter=' ')
    np.savetxt('./sim_data/P_data_all.txt', P_data_all, delimiter=' ')
    # raw data save
    ...
    raw_data_all = np.concatenate((est_pos, est_vel, est_acc), axis=0)
    np.savetxt('./sim_data/raw_data_all.txt', raw_data_all, delimiter=' ')
...
#---------------------------------------可視化結果-------------------------------------------#
    ...
    # plt.figure(1)
    plt.subplot(3, 1, 1)
    plt.plot(t, true_pos, "black", label="True Pos")
    ...
    # plt.plot(t, est_pos11, label="KF Est Pos by Q11", linewidth=1)
    plt.legend()
    plt.title("KF with different Q estimate resulr comparison")
